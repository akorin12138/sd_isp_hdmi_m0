...
set rc [catch {
  open_project {ispsim.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {tb_top}
  export_db {ispsim_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc
  optimize_rtl
  report_area -file ispsim_rtl.area
  export_db {ispsim_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea ispsim_gate.area
  legalize_phy_inst
  export_db {ispsim_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
